...
uint8_t _i2caddr = FDC2214_I2C_ADDR_0;

void FDC2214_FDC2214(uint8_t i2caddr)
{
	_i2caddr = i2caddr;
...
bool FDC2214_begin(uint8_t chanMask, uint8_t autoscanSeq, uint8_t deglitchValue, bool intOsc)
{
	I2C_Start();

	int devId = FDC2214_read16FDC(FDC2214_DEVICE_ID);
	if (devId != 0x3054)
	{
		if (devId != 0x3055)
		{
	...
	FDC2214_loadSettings(chanMask, autoscanSeq, deglitchValue, intOsc);
	//    setGain();
...
void FDC2214_loadSettings(uint8_t chanMask, uint8_t autoscanSeq, uint8_t deglitchValue, bool intOsc)
{
	...
	if (intOsc)
	{
		FDC2214_write16FDC(FDC2214_CONFIG, 0x1C81); //set config
	}
	else
	{
		FDC2214_write16FDC(FDC2214_CONFIG, 0x1E81); //set config
	}
	...
	if (chanMask & 0x01)
	{
		...
		FDC2214_write16FDC(FDC2214_SETTLECOUNT_CH0, 0x64);

		//rcount maximized for highest accuracy
		FDC2214_write16FDC(FDC2214_RCOUNT_CH0, 0xFFFF);

		//no offset
		FDC2214_write16FDC(FDC2214_OFFSET_CH0, 0x0000);
		...
		FDC2214_write16FDC(FDC2214_CLOCK_DIVIDERS_CH0, 0x2001);
		//set drive register
		FDC2214_write16FDC(FDC2214_DRIVE_CH0, 0xF800);
	}
	// Init chan2, if selected by channel init mask
	if (chanMask & 0x02)
	{
		FDC2214_write16FDC(FDC2214_SETTLECOUNT_CH1, 0x64);
		FDC2214_write16FDC(FDC2214_RCOUNT_CH1, 0xFFFF);
		FDC2214_write16FDC(FDC2214_OFFSET_CH1, 0x0000);
		FDC2214_write16FDC(FDC2214_CLOCK_DIVIDERS_CH1, 0x2001);
		FDC2214_write16FDC(FDC2214_DRIVE_CH1, 0xF800);
	}
	if (chanMask & 0x04)
	{
		FDC2214_write16FDC(FDC2214_SETTLECOUNT_CH2, 0x64);
		FDC2214_write16FDC(FDC2214_RCOUNT_CH2, 0xFFFF);
		FDC2214_write16FDC(FDC2214_OFFSET_CH2, 0x0000);
		FDC2214_write16FDC(FDC2214_CLOCK_DIVIDERS_CH2, 0x2001);
		FDC2214_write16FDC(FDC2214_DRIVE_CH2, 0xF800);
	}
	if (chanMask & 0x08)
	{
		FDC2214_write16FDC(FDC2214_SETTLECOUNT_CH3, 0x64);
		FDC2214_write16FDC(FDC2214_RCOUNT_CH3, 0xFFFF);
		FDC2214_write16FDC(FDC2214_OFFSET_CH3, 0x0000);
		FDC2214_write16FDC(FDC2214_CLOCK_DIVIDERS_CH3, 0x2001);
		FDC2214_write16FDC(FDC2214_DRIVE_CH3, 0xF800);
	}
	...
	//
	FDC2214_write16FDC(FDC2214_MUX_CONFIG, muxVal); //set mux config for channels
}
...
//*/
///**************************************************************************/
//long long NelsonsLog_FDC2214_calculateFsensor(unsigned long reading){
...
//*/
///**************************************************************************/
//double NelsonsLog_FDC2214_calculateCapacitance(long long fsensor){
...
unsigned long FDC2214_getReading28(uint8_t channel)
{
	int timeout = 100;
	unsigned long reading = 0;
	// long long fsensor = 0;
	int status = FDC2214_read16FDC(FDC2214_STATUS);
	uint8_t addressMSB;
	uint8_t addressLSB;
	...
	{
		status = FDC2214_read16FDC(FDC2214_STATUS);
		timeout--;
	}
	if (timeout == 100)
	{
		...
		reading = (uint32_t)(FDC2214_read16FDC(addressMSB) & FDC2214_DATA_CHx_MASK_DATA) << 16;
		reading |= FDC2214_read16FDC(addressLSB);
		while (timeout && !(status & bitUnreadConv))
		{
			status = FDC2214_read16FDC(FDC2214_STATUS);
			timeout--;
...
// Takes in channel number, gives out the formatted 28 bit reading.
unsigned long FDC2214_getReading16(uint8_t channel)
{
	int timeout = 100;
	unsigned long reading = 0;
	// long long fsensor = 0;
...
//*/
///**************************************************************************/
//double NelsonsLog_FDC2214_readCapacitance() {
//    int timeout = 100;
//    unsigned long reading = 0;
//    long long fsensor = 0;
//    int status = FDC2214_read16FDC(FDC2214_STATUS_REGADDR);
//    while (timeout && !(status & FDC2214_CH0_UNREADCONV)) {
////        Serial.println("status: " + String(status));
//        status = FDC2214_read16FDC(FDC2214_STATUS_REGADDR);
//        timeout--;
//    }
//    if (timeout) {
//        //read the 28 bit result
//        reading = FDC2214_read16FDC(FDC2214_DATA_CH0_REGADDR) << 16;
//        reading |= FDC2214_read16FDC(FDC2214_DATA_LSB_CH0_REGADDR);
...
*/
/**************************************************************************/
//void NelsonsLog_FDC2214_setGain(void) {
//    //todo
//}
/**************************************************************************/
/*!
...
// uint8_t FDC2214_read8FDC(uint16_t address) {
//     uint8_t data;
...
// uint32_t FDC2214_read32FDC(uint16_t address) {
//     uint32_t retVal = 0;
// 	uint8_t data;
...
// // Read 2 byte from the FDC at 'address'
// uint16_t FDC2214_FDC2214_read16FDC(uint16_t address) {
//     uint16_t data;
...
// // write 1 byte to FDC
// void FDC2214_write8FDC(uint16_t address, uint8_t data) {
//     Wire.beginTransmission(_i2caddr);
//     Wire.write(address >> 8);
//     Wire.write(address);
//     Wire.write(data);
...
// void FDC2214_FDC2214_write16FDC(uint16_t address, uint16_t data) {
//     Wire.beginTransmission(_i2caddr);
...
// Read 2 byte from the FDC at 'address'
uint16_t FDC2214_read16FDC(uint16_t address)
{
	uint16_t REG_data = 0;
	if (!I2C_Start())
		return 0;
...
// write 2 bytes to FDC
void FDC2214_write16FDC(uint16_t address, uint16_t data)
{
	// 写16位数据
	if (!I2C_Start())
		return;
	...
	I2C_SendByte((uint8_t)address); // address pointer regster data
	I2C_WaitAck();
